...
export default function generateUMLFromJSON(jsonData) {
  ...
  jsonData.classes.forEach((classData) => {
    ...
    const attributeList = attributes ? Object.entries(attributes).map(([key, value]) => `${key}: ${value}`) : [];
  ...
  jsonData.relationships.forEach((relationshipData) => {
    ...
    contentBuilder.addRelationship(from, to, relationType, label, fromMultiplicity, toMultiplicity, link);
  });
